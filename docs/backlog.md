...
A relational database would probably be [more suitable](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-general-nosql-design.html) than the NoSQL DynamoDB, even though there's only one table. It doesn't allow indexed queries on attributes of type set, so a full table scan is needed for some queries. With the current small (constrained) dataset size (924 items, 8 fields each) however, they're fast enough (50-100ms) and cheap enough (14 read capacity units @ \$0.30/million in London). As of 18 Sep 2019. Plenty of headroom for more items, more fields, and a lot of traffic.
